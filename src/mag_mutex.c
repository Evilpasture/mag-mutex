...
// Pure CPU pause. Removed sched_yield which causes severe OS stalls under high contention.
...
static_assert(ATOMIC_INT_LOCK_FREE == 2,
              "MagMutex requires always-lock-free atomics!");
static_assert(ATOMIC_POINTER_LOCK_FREE == 2,
              "MagMutex parking lot requires lock-free pointers!");
...
void mag_debug_check_pre_lock(MagMutex *m) {
  ...
  if (bits & MAG_LOCKED) {
    if (atomic_load_explicit(&m->has_owner, memory_order_acquire)) {
      ...
      plat_thread_id_t owner =
          atomic_load_explicit(&m->owner, memory_order_relaxed);
...
void mag_mutex_lock_slow(MagMutex *m) {
    ...
    for (int spin = 0; spin < MAX_SPIN_COUNT; spin++) {
        ...
        if (!(v & MAG_LOCKED) && !(v & MAG_POISONED)) {
            if (atomic_compare_exchange_weak_explicit(&m->bits, &v, v | MAG_LOCKED, memory_order_acquire, memory_order_relaxed)) {
#ifdef MAG_DEBUG
    ...
    for (;;) {
        ...
        // Re-check after acquiring bucket lock
